import { LayoutDashboard, User, FileText, Video, FileSearch, HelpCircle, LogOut, Settings, Menu, ChevronDown, X } from 'lucide-react';
    ...
    >
      ...
      <div className="flex h-16 items-center flex-shrink-0 px-4 justify-between border-b border-slate-100">
        <div className="flex items-center gap-2 overflow-hidden min-w-0">
          <div className={cn("flex items-end gap-0.5 shrink-0", collapsed && "mx-auto")}>
        ...
        {navItems.map((item) => (
          ...
          >
            ...
            {!collapsed && item.hasDropdown && (
              <ChevronDown size={16} className={cn(activeItem === item.name ? "text-white/70" : "text-slate-400")} />
            )}
          </button>
        ))}
      </nav>
